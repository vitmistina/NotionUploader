...
- **Integrations**: Provider-specific clients live in `src/services/` and the infrastructure adapters under `src/withings/`, `src/strava/`, and `src/notion/`. Shared helpers live in `src/domain/`.
...
```
.
├── src/            # Application code, routers, domain, platform wiring
